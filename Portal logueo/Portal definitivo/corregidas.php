...
<?php
	...
	if($administracion == 'Normal'){
		...
		$select = mysql_query("SELECT * FROM incidencias WHERE Usuario='".$_COOKIE['nick']."' AND Estado='CORREGIDO'", $conexion);
		//Generamos la tabla:
		echo "<table border=1 align=center>";
		//Ponemos una cabecera:
		echo "<tr><td colspan='10'><h3 align=center>Mis incidencias corregidas</h3></tr>";
		...
		while($v=mysql_fetch_row($campos)){
			foreach($v as $valor) {
				if ($valor!='Estado' AND $valor!='N_incidencia' AND $valor!='Usuario') {
					echo "<th>".$valor."</th>";
				}
			}
		}
		...
		while ($z = mysql_fetch_row ($select)) {
			echo "<tr>";
				foreach ($z as $valor) {
					$array[$a][$b]=$valor;
					if ($valor!='' AND $valor!='CORREGIDO' AND $b!=0 AND $valor!=$_COOKIE['nick']) {						
						echo "<th>".$valor."</th>";						
					}
					$b++;
				}
			if ($array[$a][4]=='CORREGIDO' AND $array[$a][6]=$_COOKIE['nick']){
	...
	if($administracion == 'Administrador'){
		...
		//Tabla incidencias no corregidas:
			...
			$select = mysql_query("SELECT * FROM incidencias WHERE Estado='CORREGIDO'", $conexion);
			//Generamos la tabla:
			echo "<table border=1 align=center>";
			//Ponemos una cabecera:
			echo "<tr><td colspan='12'><h3 align=center>Incidencias corregidas</h3>";
			//Abrimos un tr para poner los nombres de las columnas:
			echo "<tr>";
			//La siguiente instruccion y el siguiente bucle es para mostrar los nombres de los atributos al comienzo de la tabla
			$campos=mysql_query("SELECT COLUMN_NAME FROM information_schema.COLUMNS WHERE TABLE_SCHEMA = 'gestionabdera' AND TABLE_NAME = 'Incidencias'",$conexion);
