...
if(isset($_COOKIE["nick"]) AND isset($_COOKIE["pass"])) {
	...
	if(mysql_fetch_array($result)) {
		...
		if ($show=="Administrador") { //Es un usuario con privilegio de "Administrador".
			$alta_crea="INSERT INTO departamento (Nombre_departamento,Descripcion,Ubicacion)
			VALUES ('".$_POST['nombre_d']."',"."'".$_POST['descripcion_d']."','".$_POST['ubicacion_d']."');";
			$alta = mysql_query ($alta_crea, $conexion);
			//Comprobacion de que realmente se he creado el departamento
			if ($alta) {
				echo " <script type='text/javascript'>
					function redirection(){  
					window.location ='crear-departamento.php';
				...
				echo " <script type='text/javascript'>
					...
					alert('Fallo la creacion del departamento');
				</script>";
			}
		}
		else {
