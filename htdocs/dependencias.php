...
<?php
	if($administracion == 'Administrador'){
		...
		echo "<div class='col-sm-12 col-md-12 col-xl-12'><h3>Dependencias IES Abdera</h3></div><hr>";
		...
		//Tabla de departamentos:
			...
			while ($z = mysql_fetch_row ($select)) {
				...
				echo "<td class='valores'><form method='post' action='modificar-dependencia.php' align='center' name='formulario_departamento'>";
				echo "<input type='hidden' value='".$array[$a][0]."' name='mod'>";
				echo "<input name='accept' type='submit' value='M' align='center'>";
				echo "</input>";
				echo "</form></td>";
				echo "<td class='valores'><form method='post' action='script_borrar_dependencia.php' align='center' name='formulario_departamento_borrar'>";
