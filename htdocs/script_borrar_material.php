...
if(isset($_COOKIE["nick"]) AND isset($_COOKIE["pass"])) {
	...
	if(mysql_fetch_array($result)) {
		...
		if ($show=="Administrador") { //Es un usuario con privilegio de "Administrador".
			//Al ser un administrador, este usuario podra borrar cualquier material
			//Eliminacion de la incidencia
			$borrar="DELETE FROM materiales WHERE ID_material='".$_POST['baja']."';";
			$borrado=mysql_query ($borrar, $conexion);
			//Comprobacion de que realmente se ha borrado el material
			if ($borrado) {
				...
				echo " <script type='text/javascript'>
					...
					alert('Fallo en el borrado del material');
				</script>";
			}
		}
		else {
