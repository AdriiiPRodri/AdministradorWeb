...
<?php
	...
	if($administracion == 'Normal'){
		...
		echo "<div class='col-sm-12 col-md-12 col-xl-12'><h3>No corregidas</h3></div><hr>";
		...
		while ($z = mysql_fetch_row ($select)) {
			...
			if ($array[$a][4]=='NO CORREGIDO' AND $array[$a][6]=$_COOKIE['nick']){
				echo "<td class='valores'><form method='post' action='modificar-incidencia.php' align='center' name='formulario_material'>";
	...
	if($administracion == 'Administrador'){
		...
		//Tabla incidencias no corregidas:
			...
			echo "<tr>";
				foreach ($z as $valor) {
					$array[$a][$b]=$valor;
					if ($valor!='' AND $valor!='NO CORREGIDO') {						
						echo "<th class='valores'>".$valor."</th>";						
					}
					$b++;
				}
